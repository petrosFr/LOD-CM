...
do
	...
	dataset="dbpedia/2016-10/"
        ...
        #algo="apriori-linux"
        algo="fpgrowth-linux"
        mkdir -p itemsets/${dataset}/${j}/schemas
        ...
        fpm/${algo} -tm -s$minsup -m$minitem -g -v \(%s\) itemsets/${dataset}/${j}/transactions.txt itemsets/${dataset}/${j}/schemas/schema_minsup${minsup}.txt
